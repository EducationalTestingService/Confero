...
def getEventsPerFrame(session_folder, frame_times_per_session_video):
    ...
    result_dtype = [('video_id', np.uint8), ('frame_number', np.uint32),
                    ...
                    ('event_id', np.uint32), ('event_time', np.float32)]
    hubdata = None
    try:
        exp_path, session_name = session_folder.rsplit(os.path.sep, 1)
        ...
        for vi, (si, ei) in enumerate(evt_blocks_idx):
            ...
            print "Recording:",vi,len(video_event_list)
        ...
        if frame_index:
            ...
            if frame_index == 1:
                ...
                if index_diff != 1 or usec_time_diff != usec_ifi:
                    printf('checkVideoIFI alert:', frame_index, frame_time,
...
def detectSyncTimeFrames(cvid, start_frame, color_list, sync_region, iterations,
    ...
    """
    printf('STARTING: detectSyncTimeFrames: ', start_frame,
           iterations * len(color_list))
            ...
            if frame_index:
                ...
                if vframe is not None:
                    ...
                    if frame_index > start_frame + max_frames_searched:
                        printf(
                            "Error: Max frame index for detectSyncTimeFrames reached.")
                        return synctimes, frame_index
            else:
                print(
                    "Error: End of Video reached without detecting all sync frames")
                return synctimes, frame_index
    r = cvid.seek(frame_number=1)
    if r is False:
        printf("Error: Could not seek to start of video.")
    return synctimes, frame_index
...
if __name__ == '__main__':
    try:
        ...
        for rec_video_index, vpath in enumerate(session_video_paths):
            ...
            t2 = getTime()
            printf()
            printf('checkVideoIFI duration:', t2 - t1)
            printf("checkVideoIFI Issue Count:", ic)
            printf()
            ...
            t1 = getTime()
            start_sync_frames, last_frame_checked = detectSyncTimeFrames(cvid,
                                                                         start_frame=1,
            ...
            t1 = getTime()
            end_sync_frames, _ = detectSyncTimeFrames(cvid,
        ...
        if SAVE_TXT_FILE:
            ...
            header += "video_id\tframe_number\tframe_time\tevent_type\tevent_id\tevent_time\n"
            fmt = ['%d','%d','%.3f','%d','%d','%.3f']
            np.savetxt(
